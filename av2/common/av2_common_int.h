...
// `refresh_frame_flags`
// - Some other constraints for frame_type and mlayer_id are also satisfied.
...
static INLINE bool av2_frame_clears_multiple_inserted_in_one(
    int refresh_frame_flags, FRAME_TYPE frame_type, int mlayer_id,
    int *first_ref_index) {
  int refresh_count = 0;
  *first_ref_index = -1;
  ...
  // Special case is only when mlayer_id = 0.
  if (mlayer_id != 0) return false;
