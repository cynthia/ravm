...
typedef struct {
  ...
  int immediate_output_picture[MAX_SUBGOP_STATS_SIZE];
...
typedef struct AV2Decoder {
  ...
  RefCntBuffer
      *output_frames[(REF_FRAMES + 1) *
                     AVM_MAX_NUM_STREAMS];  // RefCntBuffer is used for a single
                                            // layer with the exception of when
                                            // the end of a CVS is reached.  In
                                            // that case, multiple extended
                                            // layers can be queued for output.
   ...
   * random_access_point_index is equal to random_access_point_count.
   * When not used, the default value is -1
   */
