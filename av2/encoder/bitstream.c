...
static size_t av2_write_frame_hash_metadata(
    ...
    const avm_film_grain_t *const grain_params, ObuHeader *obu_header) {
  if (!cpi->source) return 0;
  AV2_COMMON *const cm = &cpi->common;
  avm_image_t img;
  unsigned char
...
size_t av2_write_banding_hints_metadata(
  ...
  if (cpi->oxcf.tool_cfg.use_short_metadata) {
    ...
    obu_header_size =
        av2_write_obu_header(&cpi->level_params, OBU_METADATA_SHORT, 0, 0,
                             dst);
    obu_payload_size =
        av2_write_metadata_obu(metadata, dst + obu_header_size);
  } else {
    ...
    obu_header_size =
        av2_write_obu_header(&cpi->level_params, OBU_METADATA_GROUP, 0, 0,
                             dst);
    obu_payload_size = av2_write_metadata_group_header(
        dst + obu_header_size, 1, metadata);
