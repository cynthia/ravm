...
#define GET_BLOCK_MEAN(INT_TYPE, suffix)                                    \
  static double get_block_mean_##suffix(const INT_TYPE *data, int w, int h, \
                                        ...
                                        int block_size) {                   \
    const int max_h = AVMMIN(h - y_o, block_size);                          \
    const int max_w = AVMMIN(w - x_o, block_size);                          \
    double block_mean = 0;                                                  \
...
#define GET_NOISE_VAR(INT_TYPE, suffix)                                  \
  static double get_noise_var_##suffix(                                  \
    ...
    const int max_h = AVMMIN(h - y_o, block_size_y);                     \
    const int max_w = AVMMIN(w - x_o, block_size_x);                     \
    double noise_var = 0;                                                \
    double noise_mean = 0;                                               \
    for (int y = 0; y < max_h; ++y) {                                    \
      for (int x = 0; x < max_w; ++x) {                                  \
...
static void equation_system_clear(avm_equation_system_t *eqns) {
  const int n = eqns->n;
...
static void equation_system_copy(avm_equation_system_t *dst,
                                 const avm_equation_system_t *src) {
  const int n = dst->n;
...
static int equation_system_init(avm_equation_system_t *eqns, int n) {
  eqns->A = (double *)avm_malloc(sizeof(*eqns->A) * n * n);
  eqns->b = (double *)avm_malloc(sizeof(*eqns->b) * n);
  eqns->x = (double *)avm_malloc(sizeof(*eqns->x) * n);
  eqns->n = n;
...
static int equation_system_solve(avm_equation_system_t *eqns) {
  const int n = eqns->n;
  double *b = (double *)avm_malloc(sizeof(*b) * n);
  double *A = (double *)avm_malloc(sizeof(*A) * n * n);
  int ret = 0;
  if (A == NULL || b == NULL) {
...
static void equation_system_add(avm_equation_system_t *dest,
                                avm_equation_system_t *src) {
  const int n = dest->n;
  int i, j;
  for (i = 0; i < n; ++i) {
    for (j = 0; j < n; ++j) {
...
static void equation_system_free(avm_equation_system_t *eqns) {
  if (!eqns) return;
  avm_free(eqns->A);
  avm_free(eqns->b);
  avm_free(eqns->x);
...
static void noise_strength_solver_clear(avm_noise_strength_solver_t *solver) {
  equation_system_clear(&solver->eqns);
  solver->num_equations = 0;
  solver->total = 0;
}

static void noise_strength_solver_add(avm_noise_strength_solver_t *dest,
                                      avm_noise_strength_solver_t *src) {
...
static int num_coeffs(const avm_noise_model_params_t params) {
  const int n = 2 * params.lag + 1;
  switch (params.shape) {
    case AVM_NOISE_SHAPE_DIAMOND: return params.lag * (params.lag + 1);
    case AVM_NOISE_SHAPE_SQUARE: return (n * n) / 2;
  }
  return 0;
}

static int noise_state_init(avm_noise_state_t *state, int n, int bit_depth) {
  ...
  return avm_noise_strength_solver_init(&state->strength_solver, kNumBins,
                                        bit_depth);
}

static void set_chroma_coefficient_fallback_soln(avm_equation_system_t *eqns) {
...
int avm_noise_strength_lut_init(avm_noise_strength_lut_t *lut, int num_points) {
  if (!lut) return 0;
  lut->num_points = 0;
  lut->points = (double(*)[2])avm_malloc(num_points * sizeof(*lut->points));
...
static double noise_strength_solver_get_bin_index(
    const avm_noise_strength_solver_t *solver, double value) {
  const double val =
...
static double noise_strength_solver_get_value(
    const avm_noise_strength_solver_t *solver, double x) {
  const double bin = noise_strength_solver_get_bin_index(solver, x);
  const int bin_i0 = (int)floor(bin);
  const int bin_i1 = AVMMIN(solver->num_bins - 1, bin_i0 + 1);
...
int avm_noise_strength_solver_solve(avm_noise_strength_solver_t *solver) {
  ...
  for (int i = 0; i < n; ++i) {
    const int i_lo = AVMMAX(0, i - 1);
    const int i_hi = AVMMIN(n - 1, i + 1);
    A[i * n + i_lo] -= kAlpha;
    A[i * n + i] += 2 * kAlpha;
    A[i * n + i_hi] -= kAlpha;
  }
...
static void update_piecewise_linear_residual(
    const avm_noise_strength_solver_t *solver,
    const avm_noise_strength_lut_t *lut, double *residual, int start, int end) {
  const double dx = 255. / solver->num_bins;
  for (int i = AVMMAX(start, 1); i < AVMMIN(end, lut->num_points - 1); ++i) {
    const int lower = AVMMAX(0, (int)floor(noise_strength_solver_get_bin_index(
                                    solver, lut->points[i - 1][0])));
    const int upper = AVMMIN(solver->num_bins - 1,
                             (int)ceil(noise_strength_solver_get_bin_index(
                                 solver, lut->points[i + 1][0])));
    double r = 0;
    for (int j = lower; j <= upper; ++j) {
      const double x = avm_noise_strength_solver_get_center(solver, j);
...
int avm_noise_strength_solver_fit_piecewise(
    const avm_noise_strength_solver_t *solver, int max_output_points,
  ...
  if (!avm_noise_strength_lut_init(lut, solver->num_bins)) {
    fprintf(stderr, "Failed to init lut\n");
    return 0;
  }
  for (int i = 0; i < solver->num_bins; ++i) {
    lut->points[i][0] = avm_noise_strength_solver_get_center(solver, i);
  ...
  double *residual = avm_malloc(solver->num_bins * sizeof(*residual));
...
int avm_flat_block_finder_init(avm_flat_block_finder_t *block_finder,
  ...
  AtA_inv = (double *)avm_malloc(kLowPolyNumParams * kLowPolyNumParams *
                                 sizeof(*AtA_inv));
  A = (double *)avm_malloc(kLowPolyNumParams * n * sizeof(*A));
...
void avm_flat_block_finder_free(avm_flat_block_finder_t *block_finder) {
  ...
  avm_free(block_finder->A);
  avm_free(block_finder->AtA_inv);
...
int avm_flat_block_finder_run(const avm_flat_block_finder_t *block_finder,
  ...
  int num_flat = 0;
  int bx = 0, by = 0;
  double *plane = (double *)avm_malloc(n * sizeof(*plane));
  double *block = (double *)avm_malloc(n * sizeof(*block));
  index_and_score_t *scores = (index_and_score_t *)avm_malloc(
  ...
  for (by = 0; by < num_blocks_h; ++by) {
    for (bx = 0; bx < num_blocks_w; ++bx) {
      ...
      double mean = 0;
      int xi, yi;
      avm_flat_block_finder_extract_block(block_finder, data, w, h, stride,
...

      {
        ...
        const double ratio = (e1 / AVMMAX(e2, 1e-6));
...
int avm_noise_model_init(avm_noise_model_t *model,
  ...
  model->n = n;
  model->coords = (int(*)[2])avm_malloc(sizeof(*model->coords) * n);

  for (y = -lag; y <= 0; ++y) {
    const int max_x = y == 0 ? -1 : lag;
    for (x = -lag; x <= max_x; ++x) {
      switch (params.shape) {
        case AVM_NOISE_SHAPE_DIAMOND:
          if (abs(x) <= y + lag) {
            model->coords[i][0] = x;
            model->coords[i][1] = y;
            ++i;
...
static int add_block_observations(
    avm_noise_model_t *noise_model, int c, const uint8_t *const data,
  ...
  double *buffer = (double *)avm_malloc(sizeof(*buffer) * (num_coords + 1));
  ...
  for (int by = 0; by < num_blocks_h; ++by) {
    ...
    for (int bx = 0; bx < num_blocks_w; ++bx) {
      ...
      int y_end = AVMMIN((h >> sub_log2[1]) - by * (block_size >> sub_log2[1]),
...
static void add_noise_std_observations(
    avm_noise_model_t *noise_model, int c, const double *coeffs,
    ...
    int num_blocks_h) {
  const int num_coords = noise_model->n;
  avm_noise_strength_solver_t *noise_strength_solver =
      &noise_model->latest_state[c].strength_solver;

  const avm_noise_strength_solver_t *noise_strength_luma =
  ...
  for (int by = 0; by < num_blocks_h; ++by) {
    ...
    for (int bx = 0; bx < num_blocks_w; ++bx) {
      ...
      const int num_samples_h =
          AVMMIN((h >> sub_log2[1]) - by * (block_size >> sub_log2[1]),
                 block_size >> sub_log2[1]);
      const int num_samples_w =
          AVMMIN((w >> sub_log2[0]) - bx * (block_size >> sub_log2[0]),
                 (block_size >> sub_log2[0]));
      ...
      if (num_samples_w * num_samples_h > block_size) {
        ...
        const double uncorr_std = sqrt(
            AVMMAX(noise_var / 16, noise_var - pow(corr * luma_strength, 2)));
        ...
        avm_noise_strength_solver_add_measurement(
...
static int is_noise_model_different(avm_noise_model_t *const noise_model) {
  ...
  for (int c = 0; c < 1; ++c) {
    const double corr =
        avm_normalized_cross_correlation(noise_model->latest_state[c].eqns.x,
    ...
    const avm_equation_system_t *latest_eqns =
        &noise_model->latest_state[c].strength_solver.eqns;
    const avm_equation_system_t *combined_eqns =
...
static int ar_equation_system_solve(avm_noise_state_t *state, int is_chroma) {
  ...
  const double noise_var = AVMMAX(var - sum_covar, 1e-6);
  state->ar_gain = AVMMAX(1, sqrt(AVMMAX(var / noise_var, 1e-6)));
  return ret;
}
...
avm_noise_status_t avm_noise_model_update(
  ...
  if (block_size <= 1) {
    ...
    return AVM_NOISE_STATUS_INVALID_ARGUMENT;
  }
  ...
  if (num_blocks <= 1) {
    ...
    return AVM_NOISE_STATUS_INSUFFICIENT_FLAT_BLOCKS;
  }
  ...
  for (channel = 0; channel < 3; ++channel) {
    ...
    if (!add_block_observations(noise_model, channel, data[channel],
      ...
      return AVM_NOISE_STATUS_INTERNAL_ERROR;
    }
    ...
    if (!avm_noise_strength_solver_solve(
  ...
  return y_model_different ? AVM_NOISE_STATUS_DIFFERENT_NOISE_TYPE
                           : AVM_NOISE_STATUS_OK;
}
...
int avm_noise_model_get_grain_parameters(avm_noise_model_t *const noise_model,
  ...
  avm_noise_strength_lut_t scaling_points[3];
  ...
  for (int c = 0; c < 3; ++c) {
    for (int i = 0; i < scaling_points[c].num_points; ++i) {
      scaling_points[c].points[i][0] =
          AVMMIN(255, scaling_points[c].points[i][0] / strength_divisor);
      scaling_points[c].points[i][1] =
          AVMMIN(255, scaling_points[c].points[i][1] / strength_divisor);
      max_scaling_value =
          AVMMAX(scaling_points[c].points[i][1], max_scaling_value);
    }
  }
  ...
  avm_noise_strength_lut_free(scaling_points + 0);
  avm_noise_strength_lut_free(scaling_points + 1);
  avm_noise_strength_lut_free(scaling_points + 2);
  ...
  for (int c = 0; c < 3; c++) {
    avm_equation_system_t *eqns = &noise_model->combined_state[c].eqns;
    for (int i = 0; i < n_coeff; ++i) {
      max_coeff = AVMMAX(max_coeff, eqns->x[i]);
      min_coeff = AVMMIN(min_coeff, eqns->x[i]);
    }
    ...
    if (c == 0) {
      ...
      max_coeff = AVMMAX(max_coeff, y_corr[c - 1]);
      min_coeff = AVMMIN(min_coeff, y_corr[c - 1]);
    }
  }
  // Shift value: AR coeffs range (values 6-9)
  // 6: [-2, 2),  7: [-1, 1), 8: [-0.5, 0.5), 9: [-0.25, 0.25)
  film_grain->ar_coeff_shift =
      clamp(7 - (int)AVMMAX(1 + floor(log2(max_coeff)), ceil(log2(-min_coeff))),
            6, 9);
...
static float *get_half_cos_window(int block_size) {
  float *window_function =
      (float *)avm_malloc(block_size * block_size * sizeof(*window_function));
  ...
  static void dither_and_quantize_##suffix(                                 \
    ...
    for (int y = 0; y < (h >> chroma_sub_h); ++y) {                         \
      for (int x = 0; x < (w >> chroma_sub_w); ++x) {                       \
        ...
        INT_TYPE new_val = (INT_TYPE)AVMMIN(                                \
            AVMMAX(result[result_idx] * block_normalization + 0.5f, 0),     \
            block_normalization);                                           \
        const float err =                                                   \
...
int avm_wiener_denoise_2d(const uint8_t *const data[3], uint8_t *denoised[3],
  ...
  struct avm_noise_tx_t *tx_full = NULL;
  struct avm_noise_tx_t *tx_chroma = NULL;
  const int num_blocks_w = (w + block_size - 1) / block_size;
  const int num_blocks_h = (h + block_size - 1) / block_size;
  ...
  float *result = NULL;
  int init_success = 1;
  avm_flat_block_finder_t block_finder_full;
  avm_flat_block_finder_t block_finder_chroma;
  const float kBlockNormalization = (float)((1 << bit_depth) - 1);
  if (chroma_sub[0] != chroma_sub[1]) {
    fprintf(stderr,
            "avm_wiener_denoise_2d doesn't handle different chroma "
            "subsampling");
    return 0;
  }
  init_success &=
      avm_flat_block_finder_init(&block_finder_full, block_size, bit_depth);
  result = (float *)avm_malloc((num_blocks_h + 2) * block_size * result_stride *
                               sizeof(*result));
  plane = (float *)avm_malloc(block_size * block_size * sizeof(*plane));
  block =
      (float *)avm_memalign(32, 2 * block_size * block_size * sizeof(*block));
  block_d = (double *)avm_malloc(block_size * block_size * sizeof(*block_d));
  plane_d = (double *)avm_malloc(block_size * block_size * sizeof(*plane_d));
  window_full = get_half_cos_window(block_size);
  tx_full = avm_noise_tx_malloc(block_size);

  if (chroma_sub[0] != 0) {
    init_success &= avm_flat_block_finder_init(
        &block_finder_chroma, block_size >> chroma_sub[0], bit_depth);
    window_chroma = get_half_cos_window(block_size >> chroma_sub[0]);
    tx_chroma = avm_noise_tx_malloc(block_size >> chroma_sub[0]);
  } else {
      ...
      for (int offsx = 0; offsx < (block_size >> chroma_sub_w);
        ...
        for (int by = -1; by < num_blocks_h; ++by) {
          for (int bx = -1; bx < num_blocks_w; ++bx) {
            ...
            avm_flat_block_finder_extract_block(
            ...
            avm_noise_tx_forward(tx, block);
            avm_noise_tx_filter(tx, noise_psd[c]);
            avm_noise_tx_inverse(tx, block);
  ...
  avm_free(plane_d);
  avm_free(block_d);
  avm_free(window_full);

  avm_noise_tx_free(tx_full);

  avm_flat_block_finder_free(&block_finder_full);
  if (chroma_sub[0] != 0) {
    avm_flat_block_finder_free(&block_finder_chroma);
    avm_free(window_chroma);
    avm_noise_tx_free(tx_chroma);
  }
  return init_success;
}
...
struct avm_denoise_and_model_t {
  ...
  avm_noise_model_t noise_model;
};

struct avm_denoise_and_model_t *avm_denoise_and_model_alloc(int bit_depth,
                                                            int block_size,
                                                            float noise_level) {
  struct avm_denoise_and_model_t *ctx =
      (struct avm_denoise_and_model_t *)avm_malloc(
          sizeof(struct avm_denoise_and_model_t));
  if (!ctx) {
  ...
  ctx->noise_psd[0] =
      avm_malloc(sizeof(*ctx->noise_psd[0]) * block_size * block_size);
  ctx->noise_psd[1] =
      avm_malloc(sizeof(*ctx->noise_psd[1]) * block_size * block_size);
  ctx->noise_psd[2] =
      avm_malloc(sizeof(*ctx->noise_psd[2]) * block_size * block_size);
  if (!ctx->noise_psd[0] || !ctx->noise_psd[1] || !ctx->noise_psd[2]) {
    fprintf(stderr, "Unable to allocate noise PSD buffers\n");
    avm_denoise_and_model_free(ctx);
    return NULL;
  }
  return ctx;
}

void avm_denoise_and_model_free(struct avm_denoise_and_model_t *ctx) {
  avm_free(ctx->flat_blocks);
  for (int i = 0; i < 3; ++i) {
    avm_free(ctx->denoised[i]);
    avm_free(ctx->noise_psd[i]);
  }
  avm_noise_model_free(&ctx->noise_model);
  avm_flat_block_finder_free(&ctx->flat_block_finder);
  avm_free(ctx);
}

static int denoise_and_model_realloc_if_necessary(
    struct avm_denoise_and_model_t *ctx, YV12_BUFFER_CONFIG *sd) {
  ...
  ctx->denoised[0] = avm_malloc((sd->y_stride * sd->y_height) << 1);
  ctx->denoised[1] = avm_malloc((sd->uv_stride * sd->uv_height) << 1);
  ctx->denoised[2] = avm_malloc((sd->uv_stride * sd->uv_height) << 1);
  ...
  ctx->flat_blocks = avm_malloc(ctx->num_blocks_w * ctx->num_blocks_h);

  avm_flat_block_finder_free(&ctx->flat_block_finder);
  if (!avm_flat_block_finder_init(&ctx->flat_block_finder, ctx->block_size,
                                  ctx->bit_depth)) {
  ...
  const avm_noise_model_params_t params = { AVM_NOISE_SHAPE_SQUARE, 3,
                                            ctx->bit_depth };
  avm_noise_model_free(&ctx->noise_model);
  if (!avm_noise_model_init(&ctx->noise_model, params)) {
  ...
  const float y_noise_level =
      avm_noise_psd_get_default_value(ctx->block_size, ctx->noise_level);
  const float uv_noise_level = avm_noise_psd_get_default_value(
...
int avm_denoise_and_model_run(struct avm_denoise_and_model_t *ctx,
  ...
  avm_flat_block_finder_run(&ctx->flat_block_finder, data[0], sd->y_width,
                            sd->y_height, strides[0], ctx->flat_blocks);

  if (!avm_wiener_denoise_2d(data, ctx->denoised, sd->y_width, sd->y_height,
  ...
  const avm_noise_status_t status = avm_noise_model_update(
  ...
  if (status == AVM_NOISE_STATUS_OK) {
    have_noise_estimate = 1;
  } else if (status == AVM_NOISE_STATUS_DIFFERENT_NOISE_TYPE) {
    avm_noise_model_save_latest(&ctx->noise_model);
    have_noise_estimate = 1;
  } else {
  ...
  if (have_noise_estimate) {
    if (!avm_noise_model_get_grain_parameters(&ctx->noise_model, film_grain)) {
