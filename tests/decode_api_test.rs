...
fn general_non_lossless_qmatrix_uv_split_keyframe_payload() -> Vec<u8> {
    struct BitWriter {
        bytes: Vec<u8>,
        bit_offset: usize,
    }
...
#[test]
fn test_rust_backend_decodes_general_non_lossless_qmatrix_uv_split_keyframe_packet() {
    let mut decoder = Decoder::builder()
        .backend(BackendKind::Rust)
        .build()
        .expect("rust backend build");
    let seq = general_sequence_header_payload(64, 48);
    let frame = general_non_lossless_qmatrix_uv_split_keyframe_payload();
    let mut packet = vec![0x0a, seq.len() as u8];
    packet.extend_from_slice(&seq);
    packet.extend_from_slice(&[0x22, frame.len() as u8]);
    packet.extend_from_slice(&frame);
    ...
    decoder
        .decode(&packet)
        .expect("general non-lossless qmatrix uv-split keyframe decode");
    let frames: Vec<_> = decoder.get_frames().collect();
    assert_eq!(frames.len(), 1);
    let info = decoder.get_stream_info().expect("stream info");
    assert_eq!(info.width, 64);
